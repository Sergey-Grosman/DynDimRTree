...
///
    ...
    public:
        ...
        public:
            ...
            explicit Iterator() : dim_(0) { Init(0); }
            ...
            void GetBounds(ELEMTYPE * const a_min, ELEMTYPE * const a_max) {
                        ASSERT(IsNotNull());
                StackElement &curTos = m_stack[m_tos - 1];
    ...
    protected:
        ...
        bool RemoveRect(const RectRef & a_rect, const DATATYPE &a_id, Node **a_root);

        bool RemoveRectRec(const RectRef & a_rect, const DATATYPE &a_id, Node *a_node, ListNode **a_listNode);
...
// Users will likely replace its usage with a Stream implementation from their favorite API.
    ...
    RTREE_QUAL::RTree(size_t dim) : dim_(dim) {
        ...
        static const std::vector<ELEMTYPEREAL> UNIT_SPHERE_VOLUMES = {
    ...
    void RTREE_QUAL::CopyRec(Node *current, Node *other) {
        ...
        if (current->IsInternalNode())  // not a leaf node
        {
            for (int index = 0; index < current->m_count; ++index) {
                ...
                currentBranch->m_rect.dim_ = dim_;
...
// RemoveRect provides for eliminating the root.
    RTREE_TEMPLATE
    bool RTREE_QUAL::RemoveRect(const RectRef & a_rect, const DATATYPE &a_id, Node **a_root) {
                ASSERT(a_root);
                ASSERT(*a_root);
...
// Returns 1 if record not found, 0 if success.
    RTREE_TEMPLATE
    bool RTREE_QUAL::RemoveRectRec(const RectRef & a_rect, const DATATYPE &a_id, Node *a_node, ListNode **a_listNode) {
